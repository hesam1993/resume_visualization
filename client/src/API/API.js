...
async function getApplications() {
  ...
  if (response.ok) {
    ...
    const final = applications.map(
      (a) => new models.ApplicationsData(a.id, a.positionTitle, a.sumCandidates)
    );
    return final;
  } else {
...
async function getGithubInfo(githubId) {
  ...
  if (response.ok) {
    ...
    const final = new models.GithubData(githubJson.id, githubJson.public_repos);
...
async function getTeams() {
  ...
  if (response.ok) {
    ...
    const final = teams.map((a) => new models.TeamData(a.id, a.name));
    return final;
  } else {
...
async function getFields() {
  console.log("GET FIELDS IS CALLED");
  let url = `${baseURL}/fields`;
  const response = await fetch(url);
  const fieldsJson = await response.json();
  if (response.ok) {
    const fields = fieldsJson.fields;
    const final = fields.map((a) => new models.FieldData(a.id, a.field));
    return final;
  } else {
    let err = { status: response.status, errObj: fieldsJson };
    throw err; // An object with the error coming from the server
  }
}
...
async function addPosition(positionInfo) {
  var requestOptions = {
    ...
    body: JSON.stringify({
      ...
      desc: positionInfo.desc,
      minExp: parseInt(positionInfo.minExp),
      skills: positionInfo.skills,
      teamId: parseInt(positionInfo.teamId),
      fieldId: parseInt(positionInfo.fieldId),
